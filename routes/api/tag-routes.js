...
const { Tag, Product, ProductTag, Product_Tag } = require('../../models');
    ...
    const tagData = await Tag.findAll(
      {
        ...
        include: [{ model: Product, through: Product_Tag, as: 'Tag_Products' }
                 ]
      }
...
router.put('/:id', async (req, res) => {
  ...
  .then((tag) => {
    ...
    return Product_Tag.findAll({ where: { tag_id: req.params.id } });
  })
  .then((tagProducts) => {
    ...
    // run both actions
    return Promise.all([
      Product_Tag.destroy({ where: { id: tagProductsToRemove } }),
